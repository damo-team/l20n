...
export class Remote {
  ...
  registerView(viewRemoteId, resources) {
    return this.interactive.then(() => {
      if(this.langDir){
        resources.push(this.langDir + '/{locale}.json')
      }
      
      this.ctxs.set(viewRemoteId, this.env.createContext(resources));
      return true;
    });
  }
  ...
  formatValues(viewRemoteId, keys) {
    return this.languages.then(
      langs => this.ctxs.get(viewRemoteId).resolveValues(langs, keys));
  }
